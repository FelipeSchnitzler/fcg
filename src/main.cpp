...
int main(int argc, char *argv[])
{
    ...
    GLFWwindow *window;
    window = glfwCreateWindow(800, 600, "Metralhovos", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 view = Matrix_Camera_View(camera.position, camera.view, camera.up);
        ...
        else
        {
            ...
            float t = 1.5f * camera.distance / 2.5f;
            float b = -t;
...
void CursorPosCallback(GLFWwindow *window, double xpos, double ypos)
{
    ...
    if (g_LeftMouseButtonPressed)
    {
        ...
        //UpdateCameraAngle(0.01f * dx, 0.01f * dy);
        camera.UpdateCameraAngle(0.01f * dx, 0.01f * dy);
...
void TextRendering_ShowPlayerSpeed(GLFWwindow *window, Player player)
{
    ...
    // Variáveis estáticas (static) mantém seus valores entre chamadas
    // subsequentes da função!
    static char buffer[50] = "?? batidas de asas/s";
    static int numchars = 7;
    ...
    if (ellapsed_seconds > 1.0f)
    {
        numchars = snprintf(buffer, 20, "%.2f batidas de asas/s", player.speed);
