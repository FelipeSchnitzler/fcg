...
// renderização.
float g_CameraTheta = M_PI / 4; // Ângulo no plano ZX em relação ao eixo Z
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "Nome do Joguinho", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    float speed = 1.0f;
    float g_CameraX = 2.0f;
    float g_CameraY = 2.0f;
    float g_CameraZ = 2.0f;
    glm::vec4 Char_Move(1.0f,0.0f,1.0f,0.0f);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        g_CameraX += -Char_Move.x * speed/2 * delta_t;//mantem o personagem sempre em movimento
        g_CameraZ += -Char_Move.z * speed/2 * delta_t;


        if (g_Dpressed)
        {
            Char_Move.x = Char_Move.x*cos(M_PI/180) - Char_Move.z*sin(M_PI/180);//rotaciona o movimento do personagem
            Char_Move.z = Char_Move.x*sin(M_PI/180) + Char_Move.z*cos(M_PI/180);
            //TODO aq precisaria girar a camera junto com o personagem
